...
function TaskCard({ task }: { task: Task }) {
  return (
    ...
    >
      ...
      {task.priority && <View className="mb-2"><PriorityBadge priority={task.priority} /></View>}
      {task.location_name && (
        <Text className="text-xs text-gray-500 mb-1" numberOfLines={1}>📍 {task.location_name}</Text>
      )}
      {task.scheduled_at && (
      ...
      <View className="bg-white dark:bg-gray-900 px-5 pt-14 pb-3 border-b border-gray-100 dark:border-gray-800">
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Text className={`text-sm font-medium ${statusFilter === item.value ? 'text-white' : 'text-gray-600 dark:text-gray-300'}`}>
                {item.label}
              </Text>
            </Pressable>
          )}
